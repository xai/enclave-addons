...
2. Check that a review session exists: run `diffity agent list`. If this fails with "No active review session", tell the user to start it with **/diffity-diff**.
...
3. For each open thread, check the `comments` array and the `author.type` field (`"user"` or `"agent"`) on each comment:
   ...
   f. Read the comment body from the JSON output and understand what change is requested. Interpret the intent:
      ...
      - If the comment is genuinely unclear and you cannot determine what action to take, reply asking for clarification instead of silently skipping:
        ...
        ```
   g. Read the relevant source file to understand the full context around the commented lines, then make the requested change using the current tool's editing facility.
...
5. Tell the user the resolved status will appear in the existing Diffity host browser view within 2 seconds via polling.
