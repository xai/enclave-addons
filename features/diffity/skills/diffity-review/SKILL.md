...
2. If a matching entry exists, compare its `ref` field against the requested ref:
   ...
   - If refs **match** → reuse the session and continue to Step 2.
   - If refs **don't match** → restart with `diffity --no-open --new <ref>` (or `diffity --no-open --new` if no ref). Start it as a long-running background command using the current tool's shell execution facility. The `--new` flag kills the old session and starts a fresh one on the only published container port. Remember that a replacement occurred, wait 2 seconds, then verify it with `diffity list --json`.
   - If **no ref was requested** and the running session's ref is not `"work"` → restart with `diffity --no-open --new` (the running session is for a named ref, but we need working-tree) and remember that a replacement occurred.
3. If **no session is running** for this repo, start one in the background:
   - Command: `diffity --no-open <ref>` (or `diffity --no-open` if no ref). Do not use `--new` when there is nothing to replace.
   - Use the current tool's long-running shell execution facility.
   - Wait 2 seconds, then verify with `diffity list --json`. Its port is container-local; do not give it to the user.
...
2. Find and read the project instruction files applicable to the changed files, such as `AGENTS.md` or `CLAUDE.md`. These define project-specific rules that the diff must follow.
...
- Project-instruction violations where you can quote the exact rule being broken
- Broken contracts — a changed function that no longer satisfies what its callers expect
- Missing tests for new or changed behavior (as `[suggestion]`, not `[must-fix]`, unless the applicable project instructions require tests)
...
- For project-instruction violations, confirm the rule is scoped to this file
...
3. For each finding, leave an inline comment using:
   ...
   - If flagging a project-instruction violation, quote the exact rule being broken
4. After leaving all inline comments, decide whether a general comment is needed:
   - **No findings → leave a general comment:** "No issues found. Checked for bugs and project-instruction compliance."
...
2. Verify persistence: take the session's `repoHash` from `diffity list --json` and run `readlink "$HOME/.diffity/<repoHash>"`. If it prints nothing, that path is not a link into the host store, so the comments you just left are container-local and vanish with the session. Lead with that. It means the feature's link no longer matches diffity's layout — a feature bug to report, not something to repair from inside the session.
3. Build the path `/diff?ref=<ref>` from the `ref` field the session reports in `diffity list --json`, using that value verbatim (`/diff?ref=work` for a working-tree session, `/diff?ref=main..HEAD` for that comparison). The user must open this path, not the bare URL: the UI reads the ref from the query string and falls back to the working tree when it is missing, so the bare URL shows "No changes found" on a clean tree and the comments are nowhere to be seen.
4. Tell the user the review is ready and direct them to the Diffity URL Enclave printed at session start (or `enclave ps` on the host) with that path appended. If Step 1 replaced a mismatched session, say so explicitly before the review summary. Example:
